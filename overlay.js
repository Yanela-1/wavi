// overlay.js — All panel rendering for items
...
function buildEnvHTML() {
  ...
  <div class="env-ov-scene">
    ...
    <div class="lp-panel">
      ...
      ${ls.length === 0 ? `<p style="text-align:center;color:rgba(253,236,216,.28);font-size:.76rem;padding:.9rem">No letters yet. Click the pencil to write one.</p>` : ''}
      <div class="l-scroll">
...
function editL(e, i) {
  e.stopPropagation();
  APP.ei = i;
  openOv(buildEdHTML(APP.letters[i]));
}
function delL(e, i) {
  e.stopPropagation();
  if (!confirm('Delete this letter?')) return;
  APP.letters.splice(i, 1);
  APP.oli = null;
  saveApp(); updBadge(); openEnvOv();
  showT('Letter deleted.');
}

function buildEdHTML(l) {
  const ed = l !== undefined;
  return `<div class="ov-panel">
  <button class="ov-close" onclick="${ed ? 'openEnvOv()' : 'closeOv()'}">x &nbsp;close</button>
  <div class="ed-panel">
...
function saveL() {
  ...
  if (APP.ei !== null) {
    APP.letters[APP.ei] = { date: d, body: b };
    APP.ei = null;
  } else {
    APP.letters.push({ date: d, body: b });
  }
  saveApp(); updBadge();
...
function buildCamHTML() {
  ...
  return `<div class="ov-panel">
  <button class="ov-close" onclick="closeOv()" style="border-color:rgba(200,160,80,.3);color:#c8a050">x &nbsp;close</button>
  <div class="cam-panel">
    <div class="cam-hdr">
...
function openFrameOv() {
  ...
  setTimeout(renderOvFrameCanvas, 50);
}
...
function setFrm(i) {
  APP.fi = APP.fi === i ? -1 : i;
  saveApp(); refreshFrame(); openFrameOv();
}
...
function buildWkHTML() {
  ...
  return `<div class="ov-panel">
  <button class="ov-close" onclick="closeOv()" style="border-color:rgba(72,136,216,.3);color:#4888d8">x &nbsp;close</button>
  <div class="wk-panel">
...
function handleSongs(e) {
  Array.from(e.target.files).forEach(f => {
    ...
    tmp.onloadedmetadata = () => {
      APP.pl.push({ name, src, dur: fmtT(tmp.duration) });
      saveApp();
      if (APP.cv === 'walkman') {
        document.getElementById('ov-wrap').innerHTML = buildWkHTML();
        document.getElementById('song-up').onchange = handleSongs;
        setTimeout(renderOvWkCanvas, 50);
      }
...
function rmSong(e, i) {
  e.stopPropagation();
  APP.pl.splice(i, 1);
  if (APP.ti >= APP.pl.length) APP.ti = 0;
  saveApp();
  if (APP.cv === 'walkman') {
    document.getElementById('ov-wrap').innerHTML = buildWkHTML();
    document.getElementById('song-up').onchange = handleSongs;
